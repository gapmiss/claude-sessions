...
- `PermissionRequest` hook events indicate permission was requested for a tool call — rendered as shield icon in tool header. The record shape changed in CC 2.1.214: it was an `async_hook_response` attachment with `hookName: "PermissionRequest:<Tool>"` (2.1.98–2.1.117), now a `hook_permission_decision` attachment with `decision` and an authoritative `toolUseID`. Both are parsed; the new one carries no stdout/duration/command, so it drives the header indicator only
- Unknown **attachment** subtypes were silently dropped for a long time while unknown *record* types warned. That asymmetry hid the `hook_permission_decision` change — every permission approval vanished from the timeline and nothing reported it. Attachment subtypes now raise `unknown_attachment_type`. Keep the table in COMPATIBILITY.md current so the warning stays meaningful
- The parse warning is only useful if it stays quiet on healthy sessions, and only trustworthy if silencing a subtype requires a reason. `REVIEWED_ATTACHMENT_TYPES` and `REVIEWED_RECORD_TYPES` in `constants.ts` are maps, not sets, for that reason. Test: does the entry say what the record contains and why the reader doesn't need it? "Seen often" is not a reason — `companion_intro` appears once and is cosmetic, `read_truncation_notice` appears five times and explains why a tool result is incomplete
- Field names mislead in both directions. `directory` sounds like a path reference but carries a full listing; `already_read_file` sounds like content but its `content.type` is `file_unchanged` with no text; `edited_text_file` sounds like an external IDE edit but fires for any on-disk change, including one a Bash command in the same session made. Sample real values before deciding what a subtype means: `grep -rh '"type": *"attachment"' ~/.claude/projects/` piped through a JSON reader that groups by `attachment.type` and keeps one sample each
- `output_style` and `command_permissions` attachments repeat constantly (roughly 6,000 and 270 records respectively across a working vault) but are session-constant, and `command_permissions.allowedTools` is empty on ~99% of records. Collapse repeats in the parser rather than the renderer, and track the last value in a local variable — filtering the accumulated `systemEvents` array on every record is O(n²) at these counts
- A slash command's `allowed-tools` frontmatter grant lands several attachment records below the user record carrying `<command-name>`. Resolving which command asked requires walking the `parentUuid` chain, same as `async_hook_response` tool resolution
- Claude Code adds metadata-only record types over time (`mode`, `ai-title`, `agent-name`, `agent-color`, `file-history-delta`, `pr-link`, …). Each one that isn't in `SKIP_RECORD_TYPES` shows up as an "unknown record type" warning. When one appears, scan the whole corpus rather than the one reported session — `grep -rho '"type":"[a-z-]*"' ~/.claude/projects | sort -u` usually turns up several at once (it also catches nested content-block and tool-input `type` fields, so diff the result against the known record types rather than reading it raw). Add to **both** `SKIP_RECORD_TYPES` and `SKIP_TYPE_STRINGS` (the latter keeps them out of search and streaming)
...
- A message sent while Claude is working exists **only** as a `queued_command` attachment — Claude Code writes no user record for it, so dropping the attachment loses the message. `prompt` is a string, or an array of content blocks when images are attached. Two other populations share the subtype: `<task-notification>` XML (background agent results, rendered from their own records) and, occasionally, a message Claude Code also delivered as a normal prompt. De-duplicate that second case on equality or prefix, never containment — a short prompt like `npx eslint .` otherwise matches any later message quoting it
...
- A slash command's `allowed-tools` grant **cannot** override a `deny` rule in `settings.json`. Anthropic's permissions docs: "Rules are evaluated in order: deny, then ask, then allow. The first match in that order determines the outcome, and rule specificity doesn't change the order," and "If a tool is denied at any level, no other level can allow it." So a `Read(~/.ssh/**)` deny is real protection against `/statusline`'s `Read(~/**)` grant, not decoration. Three limits on that protection: deny covers Claude's file tools, the Bash file commands Claude Code recognizes (`cat`, `head`, `tail`, `sed`), and redirect targets — **not** a subprocess that opens files itself, such as a Python script; only the sandbox blocks that. In *user* settings a bare relative path resolves against `~/.claude`, so write `~/` or `//` absolute paths. A `Read` deny also blocks Edit and Write on the same path, but not NotebookEdit
...
- A text block that opens with `---` is swallowed by `MarkdownRenderer` as a YAML frontmatter delimiter — everything up to the next `---` disappears and the turn renders blank while the "Show more (N lines)" count still reports the raw line count. `normalizeMarkdown()` rewrites a leading `---` to `***`; every raw `MarkdownRenderer.render()` call on model-authored text must go through it
- `AskUserQuestion` option previews arrive in three shapes: bare ASCII, fully fenced, and (most common) fenced art followed by trailing prose. They render preformatted, not as markdown — a markdown pass collapses the whitespace that ASCII mockups depend on. `stripFenceMarkers()` drops lines that are *only* a fence, which covers all three; art like `~~~~~~~~~ <- dirt mound` survives because the trailing annotation keeps it from being a standalone fence line
